...
1. 启动做菜助手，打开“设置”。
2. 在“AI 菜谱整理（可选）”中粘贴 API Key；如使用 OpenAI 官方接口，Base URL 留空，并保留默认模型 `gpt-4o-mini`。
3. 使用第三方兼容服务时，再从服务商文档复制 Base URL 和文本模型名。
4. 点击“保存设置”。保存后 Key 不会再次显示；需要移除时，点击“清除已保存的 Key”。

也可以用记事本编辑项目目录里的 `config.json`。字段含义如下：
